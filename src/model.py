...
def down2d(in_c, out_c):  # 4x4, stride 2
...
class Encoder2D(nn.Module):
    def __init__(self):
        super().__init__()
        ch = [128, 256, 512, 1024]
        self.first = down2d(1, ch[0])  # (B,1,32,32) -> (B,128,16,16)
        stages = []
        in_c = ch[0]
        for out_c in ch[1:]:
            stages += [ResBlock2D(in_c), down2d(in_c, out_c)]
            in_c = out_c
        self.stages = nn.Sequential(*stages)               # ends at (B,1024,2,2)
...
class DimTransform(nn.Module):
    ...
    def forward(self, f2):
        ...
        f3 = self.act(self.conv3d_1x1(f3))  # (B,C,1,2,2)
        return f3
...
class Decoder3D(nn.Module):
    def __init__(self, in_c=1024):
        ...
        for out_c in ch:
            ...
            c = out_c
        self.ups = nn.Sequential(*ups)      # ends at (B,64,16,32,32)
        self.head = nn.Conv3d(c, 1, kernel_size=1)
    def forward(self, x):
        x = self.ups(x)                     # (B,64,16,32,32)
        x = self.head(x)                    # (B,1,16,32,32)
        return x.squeeze(1)                 # (B,16,32,32)
